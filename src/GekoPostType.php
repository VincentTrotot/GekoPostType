...
class GekoPostType
{
    public function __construct()
    {
        ...
        add_filter('manage_edit-vt_geko_columns', [$this, 'editColumns']);
    ...
    public function createPostype()
    {
        $labels = [
            'name' => _x('Géko', 'geko'),
            'all_items' => __('Tous les Géko'),
            'singular_name' => _x('Géko', 'geko'),
            'add_new' => _x('Ajouter un magazine', 'geko'),
        ...
        $args = [
            ...
            'taxonomies' =>[ 'vt_geko_category', 'post_tag']
        ];

        register_post_type('vt_geko', $args);
    }

    /**
     * Paramétrage des colonnes  \
     * hook: manage_edit-vt_geko_columns
     */
    ...
    public function displayMetabox()
    {
        ...
        $context['post'] = new Geko();
        $context['nonce'] = wp_create_nonce('vt_geko-nonce');
    ...
    public function save()
    {
        ...
        if (!isset($_POST['vt_geko-nonce'])) {
            return;
        }

        if (!wp_verify_nonce($_POST['vt_geko-nonce'], 'vt_geko-nonce')) {
            return $post->ID;
        }
